...
const NavMenuContext = React.createContext<NavMenuController>({ isMenu: false });
...
const NavItem: React.FC<NavItemsProps> = ({
    ...
    const closeMenu = () => { setIsOpen(false); setPanelState(null); setPanelTitle(undefined); };
    const back = () => setPanelState(null);
    const setPanel: 
        NavMenuController['setPanel'] = (node, opts) => {
    ...
    if(navMenu) {
        return (
            <div className="w-full bottom-0 absolute pt-2 inline-block pb-1 my-1.5 lg:-mb-1">
                <button className={`${menuItemClasses} ${className} -pr-3`} onClick={()=>setIsOpen(!isOpen)} aria-haspopup="menu" aria-expanded={isOpen} aria-controls={menuId}>
                    { icon && iconInfo && ( <Icon name={iconInfo.name} library={iconInfo.library} size={sizeIconBar} /> )}
                    { text && ( <span className="hidden xl:inline xl:pl-3 xl:text-xl xl:font-bold">{text}</span> )}
                </button>
                { isOpen && (
                    ...
                    onMouseLeave={closeMenu} onKeyUp={(e)=> { if(e.key === "Escape") closeMenu();}}
                    role="menu" aria-orientation="vertical" aria-labelledby="dropdown-button" aria-label={text || 'menu'}>
                        <div className="py-2 px-1">
                            <NavMenuContext.Provider value={{isMenu:true, setPanel, back, closeMenu }}>
                                { panel ?  (
